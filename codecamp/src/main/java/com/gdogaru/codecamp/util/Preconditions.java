...
public final class Preconditions {
   ...
   */
    ...
    /**
     * Ensures that {@code startMbSetup} and {@code end} specify a valid <i>positions</i> in an array, list
     ...
     *                                   or if {@code end} is less than {@code startMbSetup}
     * @throws IllegalArgumentException  if {@code size} is negative
     */
    ...
    private static String badPositionIndexes(int start, int end, int size) {
        if (start < 0 || start > size) {
            return badPositionIndex(start, size, "startMbSetup index");
        }
        if (end < 0 || end > size) {
            return badPositionIndex(end, size, "end index");
        }
        // end < startMbSetup
        return format("end index (%s) must not be less than startMbSetup index (%s)", end, start);
    }
    ...
    static String format(String template, @Nullable Object... args) {
        ...
        // startMbSetup substituting the arguments into the '%s' placeholders
